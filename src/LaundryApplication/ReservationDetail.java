...
public class ReservationDetail extends javax.swing.JFrame {
    ...
    public ReservationDetail(Transaction trans, AdminMenu admin) {
        initComponents();
        
        this.trans = trans;
        this.admin = admin;
        
        boolean status = trans.isStatus();
        String statusText = status ? "Sudah" : "Belum";
        boolean orderStatus = trans.getCustomer().getOrder().isStatus();
        String orderStatusText = orderStatus ? "Sudah" : "Belum";
        
        name_label.setText(trans.getCustomer().getName());
        phone_label.setText(trans.getCustomer().getPhoneNumber());
        type_label.setText(trans.getCustomer().getOrder().getServiceType());
        weight_label.setText(trans.getCustomer().getOrder().getWeight() + " kg");
        price_label.setText("Rp"+trans.getCustomer().getOrder().getTotalPrice());
    ...
    private void initComponents() {
        ...
        setSize(new java.awt.Dimension(682, 365));
    ...
    private void proccess_btnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_proccess_btnActionPerformed
        trans.getCustomer().getOrder().setStatus(true);
        JOptionPane.showMessageDialog(null, "Pesanan atas nama "
                            + trans.getCustomer().getName() + 
                            " sudah diselesaikan.");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ReservationDetail.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
